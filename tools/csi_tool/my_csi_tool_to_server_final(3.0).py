...
g_radio_header_pd = pd.DataFrame(np.zeros([10, len(CSI_DATA_COLUMNS_NAMES[1:-1])], dtype=np.int32),
                                 columns=CSI_DATA_COLUMNS_NAMES[1:-1])
...
class DataGraphicalWindow(QMainWindow, Ui_MainWindow):
    ...
    def command_collect_target_start(self):
        ...
        # 显示文件名预览
        self.textBrowser_log.append(
            f"<font color='cyan'>文件将保存为: {target}_{user_name}_{self.collect_counters[user_name][target]:02d}.csv</font>")

        # 显示服务器状态
        if self.checkBox_server_save.isChecked():
            self.textBrowser_log.append(
                f"<font color='cyan'>数据将同时保存到服务器: {self.lineEdit_server_url.text()}</font>")
        else:
            self.textBrowser_log.append(
                f"<font color='yellow'>注意: 服务器保存功能已禁用，数据仅保存在本地</font>")
    ...
    def process_next_collection(self):
        if self.current_collect_count < self.target_collect_count:
            print(
                f"处理下一次采集: 当前计数 {self.current_collect_count}, 目标次数 {self.target_collect_count}")  # 修改日志
            self.command_collect_target_start()
        else:
    ...
    def finish_collection(self):
        ...
        if user_name in self.collect_counters and target in self.collect_counters[user_name]:
            count = self.collect_counters[user_name][target]

            # 显示采集完成的详细信息
            self.textBrowser_log.append(
                f"<font color='green'>采集完成：用户 {user_name} 的 {target} 动作共 {count} 次</font>")

            # 显示文件保存位置
            folder_path = f"data/{target}"
            self.textBrowser_log.append(
                f"<font color='green'>数据已保存到目录: {os.path.abspath(folder_path)}</font>")

            # 显示最后一个文件名
            last_file = f"{target}_{user_name}_{count:02d}.csv"
            self.textBrowser_log.append(
                f"<font color='green'>最新文件: {last_file}</font>")

            # 显示服务器状态
            if self.checkBox_server_save.isChecked():
                self.textBrowser_log.append(
                    f"<font color='green'>数据已同步到服务器: {self.lineEdit_server_url.text()}</font>")

            # 显示下一次采集的序号预告
            next_count = count + 1
            self.textBrowser_log.append(
                f"<font color='cyan'>下一次采集将使用序号: {next_count:02d}</font>")
...
def serial_handle(queue_read, queue_write, port):
    ...
    # 添加缓冲区大小限制
    max_buffer_size = 5000  # 最大缓冲区大小限制
    buffer_warning_threshold = 4000  # 警告阈值
    last_buffer_warning_time = 0  # 上次警告时间
                        ...
                        if data_series['type'] == 'CSI_DATA':
                            with packet_lock:
                                ...
                                if data_series['taget'] != 'unknown' and current_task_id:
                                    # 检查缓冲区大小
                                    if len(task_data_buffer) >= max_buffer_size:
                                        curr_time = time.time()
                                        if curr_time - last_buffer_warning_time > 5:
                                            print(
                                                f"警告: 任务数据缓冲区已达到最大大小限制 ({max_buffer_size}条)，丢弃旧数据")
                                            last_buffer_warning_time = curr_time
                                        # 丢弃最旧的20%数据
                                        discard_count = int(max_buffer_size * 0.2)
                                        task_data_buffer = task_data_buffer[discard_count:]
                                        print(f"丢弃了 {discard_count} 条旧数据")
                                    elif len(task_data_buffer) >= buffer_warning_threshold:
                                        curr_time = time.time()
                                        if curr_time - last_buffer_warning_time > 10:
                                            print(
                                                f"警告: 任务数据缓冲区接近最大大小限制 ({len(task_data_buffer)}/{max_buffer_size})")
                                            last_buffer_warning_time = curr_time
                                    ...
                                    # 写入数据到对应的文件
                                    try:
                                        if file_key in created_files and created_files[file_key]:
